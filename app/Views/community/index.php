...
if (!function_exists('community_format_body')) {
    function community_format_body(string $text): string
    {
        ...
        foreach ($parts as $part) {
            ...
            $first = $part[0];
            if ($first === '@') {
                $out .= '<span style="color:' . \App\Helpers\ThemeHelper::getSecondary() . '; font-weight:600;">' . htmlspecialchars($part, ENT_QUOTES, 'UTF-8') . '</span>';
            } elseif ($first === '#') {
        ...
        <aside style="flex:0 0 220px; min-width:220px; max-width:240px;">
            <div style="border-radius:16px; border:1px solid #272727; background:#111118; padding:10px 12px; margin-bottom:10px;">
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
                    ...
                    ?>
                    <div style="width:56px; height:56px; border-radius:50%; background:radial-gradient(circle at 30% 20%, #fff 0, #ffb74d 30%, <?= \App\Helpers\ThemeHelper::getPrimary() ?> 65%, <?= \App\Helpers\ThemeHelper::getBackground() ?> 100%); display:flex; align-items:center; justify-content:center; font-weight:700; font-size:24px; color:<?= \App\Helpers\ThemeHelper::getBackground() ?>; box-shadow:0 0 18px rgba(<?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) ?>,<?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) ?>,<?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) ?>,0.7);">
                ...
                <nav style="display:flex; flex-direction:column; gap:4px; font-size:12px;">
                    <?php foreach ($tabs as $tabKey => $tabLabel): ?>
                        ...
                        <a href="/comunidade?tab=<?= urlencode($tabKey) ?>" style="
                            ...
                            text-decoration:none; border:1px solid <?= $isActive ? \App\Helpers\ThemeHelper::getSecondary() : 'transparent' ?>;
            ...
            <?php if ($activeTab === 'perfil'): ?>
                <div style="border-radius:16px; border:1px solid #272727; background:#111118; padding:12px 14px; font-size:13px;">
                    ...
                    <div style="margin-top:10px; font-size:11px; color:#8d8d8d;">
                        Quer editar esses dados? Vá em <a href="/conta" style="color:<?= \App\Helpers\ThemeHelper::getSecondary() ?>; text-decoration:none;">Minha conta</a>.
                    </div>
                </div>
            ...
            <?php else: ?>
                ...
                <?php elseif ($usingTagFilter && !empty($tag)): ?>
                    <div style="margin-bottom:8px; font-size:12px; color:#b0b0b0;">
                        ...
                        <a href="/comunidade" style="color:<?= \App\Helpers\ThemeHelper::getSecondary() ?>; text-decoration:none; margin-left:4px;">Limpar filtro</a>
                    </div>
                <?php endif; ?>
                        ...
                        <div id="post-<?= $postId ?>" style="border-radius:12px; border:1px solid #272727; background:#111118; padding:8px 10px; font-size:13px;">
                            ...
                            <div style="display:flex; align-items:center; gap:10px; font-size:11px; color:#b0b0b0; margin-top:4px;">
                                <form action="/comunidade/curtir" method="post" style="display:inline;">
                                    <input type="hidden" name="post_id" value="<?= $postId ?>">
                                    <button type="submit" style="background:none; border:none; color:<?= $isLiked ? \App\Helpers\ThemeHelper::getSecondary() : '#b0b0b0' ?>; cursor:pointer; padding:0;">
                                        ❤️ <?= $likes ?>
                                    </button>
                                </form>
