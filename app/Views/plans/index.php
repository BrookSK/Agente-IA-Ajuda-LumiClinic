    ...
    <?php if ($isPaidPlanWithLimit): ?>
        ...
        ">
            ...
            <a href="/tokens/comprar" style="
                ...
                padding: 9px 14px;
                border-radius: 999px;
                border: 1px solid rgba(<?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) ?>, <?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) ?>, <?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) ?>, 0.28);
        ...
        (function () {
            ...
            function setActive(cycle) {
                buttons.forEach(function (b) {
                    ...
                    b.style.background = isActive ? '<?= \App\Helpers\ThemeHelper::getButtonGradient() ?>' : 'transparent';
    ...
    <div id="plans-grid" style="display:flex; flex-direction:column; gap: 12px;">
        <?php foreach ($displayPlans as $plan): ?>
            <?php
                ...
                $cardBorder = $isFeatured ? 'rgba(' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) . ',0.55)' : 'var(--border-subtle)';
                $cardShadow = $isFeatured ? '0 0 0 1px rgba(' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) . ',0.25), ' . 'var(--shadow-card-strong)' : 'var(--shadow-card)';
                $ctaBg = $isFeatured ? \App\Helpers\ThemeHelper::getButtonGradient() : 'var(--surface-subtle)';
                $ctaColor = $isFeatured ? \App\Helpers\ThemeHelper::getBackground() : 'var(--text-primary)';
                $ctaBorder = $isFeatured ? 'none' : '1px solid var(--border-subtle)';

                if ($isCurrent) {
                    $cardBorder = 'rgba(' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) . ',0.70)';
                    $cardShadow = '0 0 0 2px rgba(' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) . ',0.22), ' . 'var(--shadow-card-strong)';
                }
                ...
                $iconBg = $isFeatured ? 'rgba(' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) . ',' . hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) . ',0.92)' : 'var(--surface-subtle)';
                $iconColor = $isFeatured ? \App\Helpers\ThemeHelper::getBackground() : 'var(--text-primary)';
            ?>
            ...
            ">
                <?php if ($isFeatured): ?>
                    <div style="position:absolute; left:50%; top:-12px; transform:translateX(-50%);">
                        <div style="background:<?= \App\Helpers\ThemeHelper::getPrimary() ?>; color:<?= \App\Helpers\ThemeHelper::getBackground() ?>; font-size:11px; font-weight:800; padding:5px 10px; border-radius:999px; box-shadow: 0 10px 24px rgba(<?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) ?>, <?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) ?>, <?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) ?>, 0.28);">
                            Mais popular
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <div style="display:flex; align-items:center; justify-content:space-between; gap: 10px; margin-bottom: 8px;">
                    ...
                    <?php if ($isCurrent): ?>
                        <div style="font-size:10px; padding:3px 9px; border-radius:999px; background:rgba(<?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) ?>, <?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) ?>, <?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) ?>, 0.12); border:1px solid rgba(<?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 1, 2)) ?>, <?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 3, 2)) ?>, <?= hexdec(substr(\App\Helpers\ThemeHelper::getPrimary(), 5, 2)) ?>, 0.28); color: var(--accent); font-weight:900;">
                            Seu plano atual
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div style="display:flex; align-items:flex-end; gap: 8px; margin-bottom: 8px;">
                    ...
                    <?php else: ?>
                        <div style="font-size: 18px; font-weight: 900; color: <?= \App\Helpers\ThemeHelper::getPrimary() ?>;">R$ <?= htmlspecialchars($priceNumber) ?></div>
                    ...
                    <ul style="list-style: none; padding-left: 0; margin: 0 0 12px 0; font-size: 12px; color: var(--text-secondary);">
                        <?php foreach ($benefits as $b): ?>
                            <li style="display: flex; gap: 8px; margin-bottom: 6px; align-items:flex-start;">
                                <span style="color: <?= \App\Helpers\ThemeHelper::getPrimary() ?>; line-height: 1.2;">✔</span>
