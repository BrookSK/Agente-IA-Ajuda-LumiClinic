...
<div style="max-width: 1000px; margin: 0 auto;">
    ...
    <?php if (isset($_GET['debug'])): ?>
        <div style="background:#111118; border-radius:12px; padding:12px 14px; border:1px solid #ff6f60; font-size:12px; color:#ffcc80; margin-bottom:12px;">
            <strong>Debug Info:</strong><br>
            Total personalities: <?= count($personalities ?: []) ?><br>
            <?php if ($personalities): ?>
                <?php foreach ($personalities as $p): ?>
                    - <?= htmlspecialchars($p['name']) ?> (Active: <?= $p['active'] ? 'YES' : 'NO' ?>)<br>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
    <?php endif; ?>
        ...
        <div class="persona-stage">
            ...
            ">
                <?php foreach ($personalities as $persona): ?>
                    <?php
                        // Temporarily show all personalities for debugging
                        ...
                        <div style="padding:10px 12px 12px 12px;">
                            ...
                            <?php else: ?>
                                <div class="persona-card-muted">
                                    <?php if (!$isActive): ?>
                                        Esta personalidade está desativada pelo administrador.
