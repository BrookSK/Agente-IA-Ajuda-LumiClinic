                    ...
                    <button type="button" class="persona-card-btn persona-default-card<?= $currentDefaultPersonaId === $pid ? ' persona-default-card--active' : '' ?>" data-persona-id="<?= $pid ?>" <?= $isComingSoon ? 'disabled' : '' ?> style="cursor:<?= $isComingSoon ? 'not-allowed' : 'pointer' ?>;">
                        ...
                        <div style="padding:10px 12px 12px 12px;">
                            <div style="display:flex; align-items:center; justify-content:space-between; gap:6px; margin-bottom:4px;">
                                ...
                                <?php if ($isComingSoon): ?>
                                    <span style="font-size:9px; text-transform:uppercase; letter-spacing:0.14em; border-radius:999px; padding:2px 7px; background:#201216; color:#ffcc80; border:1px solid <?= \App\Helpers\ThemeHelper::getSecondary() ?>; flex-shrink:0;">Em breve</span>
                                <?php endif; ?>
                            </div>
                            <?php if ($parea !== ''): ?>
